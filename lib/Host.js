    ...
    return me.find(recipient)
        .then(function (url) {
          ...
          if (conn) {
            ...
            throw new Error('Host of peer ' + recipient + ' could not be reached (' + url + ')');
          }
        });
  };
}
 ...
 * @private
 */
Host.prototype._openConnection = function(url) {
  var me = this;
...
Host.prototype.close = function () {
  ...
  if (this.server) {
    // close a connection by it's url
    ...
    return Promise.all(urls.map(closeConnection)).then(closeHost);
  }
  else {
 ...
 * @private
 */
Host.prototype._closeConnection = function (url) {
