    ...
    me.server = new WebSocketServer({port: port}, function () {
      ...
      me.server.on('connection', function(conn) {
        ...
        conn.onclose = function () {
          ...
          me._removeConnection(url, forgetPeers);
        };
...
Host.prototype._onRequest = function (conn, request) {
  ...
  switch (request.method) {
    ...
    case 'goodbye':
      ...
      this._removeConnection(url, forgetPeers);
...
/**
 * Remove a connection from the list with connections.
 * NOTE: The connection will not be closed, that must be done separately!
 * @param {String} url
 * @param {boolean} [forgetPeers=false]  If true, all peers located on the
 *                                       closed connection are removed from
 *                                       cache.
 * @private
 */
Host.prototype._removeConnection = function (url, forgetPeers) {
