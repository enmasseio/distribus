...
function Host(options) {
  ...
  this._send = function (sender, recipient, message) {
    ...
    var peer = me.peers[recipient];
    if (peer) {
      peer.emit('message', sender, message);
      return Promise.resolve(null);
    }

    // TODO: if a peer no longer exists, it should be removed from cache
    ...
    return me.find(recipient)
        .then(function (url) {
          ...
          if (conn) {
            ...
            throw new Error('Peer ' + recipient + ' could not be reached (host: ' + url + ')');
          }
        });
  };
...
/**
 * Apply configuration options to the host, and retrieve the current
 * configuration.
...
Host.prototype.create = function (id) {
  ...
  var peer = new Peer(id, this._send);
  this.peers[id] = peer;
  ...
  return new Promise(function (resolve, reject) {
    ...
    me.server.on('connection', function (conn) {
      ...
      conn.onclose = function () {
        // remove this connection from the connections list (we do not yet
        // forget the cached peers)
