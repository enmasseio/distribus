...
function Host() {
  ...
  var addresses = {}; // cached addresses of remote peers
...
Host.prototype._onRequest = function (conn, request) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    switch (request.method) {
      ...
      case 'goodbye':
        me._removeConnection(conn);
        // TODO: forget this host (on connection error, the host should be remembered and reconnected later)
        resolve('goodbye');
        break;

      case 'hosts':
        // return a list with the urls of all known hosts
        resolve(Object.keys(me.connections));

        // connect to all newly retrieved urls
        if (request.params && request.params.urls) {
          me.join(request.params.urls);
        }
        break;

      case 'find': // find a peer
        var id = request.params && request.params.id;
        resolve(me.peers[id] ? me.url : null);
        break;
      ...
      case 'send':
        ...
        if (peer) {
          ...
          reject(new Error('Peer not found (id' + to + ')'));
        }
        break;
...
/**
 * Join an other Host.
 * @param {string | string[]} urls For example ['ws://localhost:300']
 * @return {Promise.<Host, Error>} Returns itself when joined
 */
Host.prototype.join = function (urls) {
  if (typeof urls === 'string') {
    urls = [urls];
  }

  var me = this;
  var inProgress = 0;     // number of joins currently in progress
  var maxInProgress = 10; // maximum number of concurrent joins

  return new Promise(function (resolve, reject) {
    /**
     * Join a single host, and retrieve the list with hosts that this host knows
     * @param {String} url
     * @return {Promise.<Array.<String>, Error>} urls
     */
    function join(url) {
      return new Promise(function (resolve, reject) {
        var conn = me.connections[url];
        if (conn) {
          ...
          conn.onopen = function () {
            ...
            conn.request({method: 'greeting', params: { url: me.url } })
                .then(function () {
                  // ask a list with known hosts
                  // TODO: send the hosts I know about to the connected host. This gives circular loops though
                  return conn.request({'method': 'hosts'});
                  //return conn.request({'method': 'hosts', params: { urls: Object.keys(me.connections) }});
                })
                .then(function (urls) {
                  resolve(urls);
                })
    ...
    /**
     * Join all urls currently in the queue
     */
    function next() {
      // loop over all queued urls and join them
      while(urls.length && inProgress < maxInProgress) {
        var url = urls.shift();
        inProgress++;
        join(url)
            .then(function (newUrls) {
              if (newUrls && newUrls.length) {
                // add newly found urls to the queue
                newUrls.forEach(function (url) {
                  if (!me.connections[url] && url !== me.url) urls.push(url);
                });
              }
            })
            .catch(function (err) {
