        ...
        .then(function (url) {
          ...
          if (conn) {
            ...
            //return conn.notify(request) // the send request returns null
                .catch(function (err) {
                  // FIXME: use a protocol for errors, error should be an Error, not a string
                  if (err.indexOf('Error: Peer not found') === 0) {
                    // this peer was deleted. Remove it from cache
                    delete me.addresses[recipient];
                    throw _peerNotFoundError(recipient);
                  }
                })
          }
          else {
            throw _peerUnreachable(recipient, url);
          }
        });
  };
